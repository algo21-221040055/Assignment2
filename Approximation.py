...
class Approximation(BaseEstimator, TransformerMixin):
    ...
    def transform(self, price: pd.DataFrame, y=None) -> pd.DataFrame:
        ...
        dif, dea, histogram = talib.MACD(price['close'].values.astype(float),fastperiod=self.fastperiod,slowperiod=self.slowperiod,signalperiod=self.signalperiod)

        atr = talib.ATR(price['high'].values, price['low'].values, price['close'].values, self.N)
        df = pd.DataFrame([])
